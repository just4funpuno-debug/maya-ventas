/**
 * Script de Migración: Team Messages
 * Fase 1.2: Migra colección 'team_messages' de Firebase a Supabase
 * 
 * Uso: node scripts/migrate-team-messages.js
 */
...
// Función para convertir readBy array
function convertReadBy(value) {
  if (Array.isArray(value)) {
    return value; // Ya es array, retornar tal cual
  }
  return []; // Si no es array, retornar array vacío
}

async function migrateTeamMessages() {
  logger.phaseStart('Migración de Team Messages', '1.2');
  
  try {
    // 1. Leer todos los documentos de Firebase
    logger.info('📖 Leyendo documentos de Firebase...');
    const snapshot = await db.collection('team_messages').get();
    const firebaseDocs = [];
    ...
    for (const doc of firebaseDocs) {
      try {
        ...
        const supabaseData = {
          ...
          author_id: doc.authorId || null, // Puede ser null si no existe el usuario aún
          author_nombre: doc.authorNombre || doc.author_nombre || null,
          text: doc.text || '',
          read_by: convertReadBy(doc.readBy || doc.read_by || []),
    ...
    logger.subphaseEnd('Migración de Team Messages', '1.2');
